    ...
    public function register(Request $request)
    {
        ...
        try {
            ...
            if ($request->filled('referral_code')) {
                \Log::info('Referral Code Provided:', ['code' => $request->referral_code]);
            
                $referrer = User::where('referral_code', $request->referral_code)->first();
            
                if ($referrer) {
                    \Log::info('Referrer Found:', ['referrer_id' => $referrer->id, 'referrer_code' => $referrer->referral_code]);
                    $referredBy = $referrer->id;
                } else {
                    \Log::warning('Referral Code Not Found:', ['code' => $request->referral_code]);
                }
            }            
            ...
            $user = User::create([
                ...
                'referred_by' => $referredBy, // Assign referrer ID
                'referral_code' => Str::random(8), // Generate unique referral code
            ]);

            if ($referredBy) {
                $user->referred_by = $referredBy;
                $user->save(); // Ensure it is saved
            }
    
            \Log::info('New User:', ['user' => $user]);
